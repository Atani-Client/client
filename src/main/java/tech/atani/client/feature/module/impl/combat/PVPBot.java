...
public class PVPBot extends Module {
    ...
    public void onPostTickEvent(PostTickEvent postTickEvent) {
        ...
        List<EntityLivingBase> targets = FightUtil.getMultipleTargets(150, players.getValue(), animals.getValue(), walls.getValue(), monsters.getValue(), invisible.getValue());
        ...
        switch (targetMode.getValue()) {
            case "Hybrid":
                ...
                break;
            case "Single":
                if (curEntity == null || !FightUtil.isValid(curEntity, 150, players.getValue(), animals.getValue(), monsters.getValue(), invisible.getValue()))
                    curEntity = targets.get(0);
                break;
            case "Multi":
            case "Switch":
                ...
                if (curEntity != null && FightUtil.isValid(curEntity, 150, players.getValue(), animals.getValue(), monsters.getValue(), invisible.getValue()) && targets.size() == 1) {
                    return;
    ...
    public final void onRotation(RotationEvent rotationEvent) {
        ...
        if (curEntity != null) {
            ...
            // Literally just killaura but i changed some numbers:D
