package wtf.atani.utils.render.shader.legacy;
...
import wtf.atani.utils.os.FileUtil;

public class LegacyShader implements Methods {
    private final int programID;

    public LegacyShader(String fragmentShaderLoc, String vertexShaderLoc) {
    ...
    public LegacyShader(String fragmentShadersrc, boolean notUsed) {
        int program = glCreateProgram();
        int fragmentShaderID = createShader(new ByteArrayInputStream(fragmentShadersrc.getBytes()), GL_FRAGMENT_SHADER);
        int vertexShaderID = 0;
        try {
            vertexShaderID = createShader(mc.getResourceManager().getResource(new ResourceLocation("atani/shaders/vertex/vertex.vsh")).getInputStream(), GL_VERTEX_SHADER);
        } catch (IOException e) {
    ...
    public LegacyShader(String fragmentShaderLoc) {
        this(fragmentShaderLoc, "atani/shaders/vertex/vertex.vsh");
    }
